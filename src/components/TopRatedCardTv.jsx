...
function TopRatedCardTv({ top }) {
  const vote = top.vote_average.toFixed(1); // Convert to a string with one decimal place
  const vote_average = vote.slice(0, 3); // Slice the first 3 characters
  // Input date string in the "YYYY-MM-DD" format
  const inputDateStr = top.first_air_date;
